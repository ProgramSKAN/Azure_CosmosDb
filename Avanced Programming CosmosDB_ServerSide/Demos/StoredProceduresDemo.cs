...
namespace CosmosDb.ServerSide.Demos
{
	...
	public static class StoredProceduresDemo
	{
		...
		private async static Task Execute_spHelloWorld()
		{
			...
			//.Scripts property gives access to container's stored procedures, triggers and user defined functions
			var scripts = Shared.Client.GetContainer("mydb", "mystore").Scripts;//"adventure-works","stores"
			var pk = new PartitionKey(string.Empty);
			//all SPs are scoped to single partiton key. that determines which document to be accessed by the SP.
			//since HelloWorld SP is not reading/writing any documents so provide empty here for partiton key

			var result = await scripts.ExecuteStoredProcedureAsync<string>("spHelloWorld", pk, null); //or dynamic type
		...
		//Stored Procedure with document Validation and maipulation
		private async static Task Execute_spSetNorthAmerica1()
		{
			...
			var result = await container.Scripts.ExecuteStoredProcedureAsync<dynamic>("spSetNorthAmerica", pk, new[] { documentDefinition, true });//true > enforcedSchema (passed as argument to javascript function)
		...
		private async static Task Execute_spSetNorthAmerica3()
		{
			...
			try
			{
				// Should fail with no country and enforceSchema = true (since no countryRegionName property beneath address)
				var result = await container.Scripts.ExecuteStoredProcedureAsync<dynamic>("spSetNorthAmerica", pk, new[] { documentDefinition, true });
			}
