...
{
	//4. Custom Indexing Policies to finetune performace
	public static class IndexingDemo
	{
		...
		private async static Task ExcludedPaths()
		{
			...
			containerProps.IndexingPolicy.ExcludedPaths.Add(new ExcludedPath { Path = "/miscellaneous/*" });//miscellaneous property and every property bebeath it is not indexed
			containerProps.IndexingPolicy.IncludedPaths.Add(new IncludedPath { Path = "/miscellaneous/rating/?" });//but rating property and only that property should be indexed

			await Shared.Client.GetDatabase("mydb").CreateContainerAsync(containerProps, 400); //"adventure-works"

			// Load documents 
			var container = Shared.Client.GetContainer("mydb", "customindexing");

			for (var i = 1; i <= 100; i++)//populate container with 100 documents
			{
				dynamic doc = new
				{
			...
			//you must need an index to sort
		...
		private async static Task CompositeIndexes()
		{
			...
			var sql = @"
				...
				ORDER BY
					...
					c.name
			";
			//to make ORDER BY work on 3 columns, COMPOSITE INDEX on above three properties is must.
			//else composite index missing error

			var container = Shared.Client.GetContainer("mydb", "mystore");//"adventure-works","store"
			...
			var compositePaths = new Collection<CompositePath>// we have define sort order also fro composite indexing
			{
		...
		//if properties in a document has spatial data then keep spatial indexes on those props
		private async static Task SpatialIndexes()
		{
			...
			//lines and polygons also to be spatial indexed

			await Shared.Client.GetDatabase("mydb").CreateContainerAsync(containerDef, 1000);//"adventure-works"
			var container = Shared.Client.GetContainer("mydb", "spatialindexing");

			for (var i = 1; i <= 1000; i++)// test data
			{
				...
				dynamic doc = new
				{
					...
					title = $"Document {i}",
					state,
					geo1 = new //sptial indexed
					{
						type = "Point",
						coordinates = new[] { longitude, latitude },
					},
					geo2 = new //no spatial index
					{
						type = "Point",
						coordinates = new[] { longitude, latitude },
					},
			...
			var result = await container.GetItemQueryIterator<dynamic>(sql).ReadNextAsync();//6RUs, since spatial indexed
			...
			result = await container.GetItemQueryIterator<dynamic>(sql).ReadNextAsync();//22RUs , no spatial index
			list = result.ToList();
