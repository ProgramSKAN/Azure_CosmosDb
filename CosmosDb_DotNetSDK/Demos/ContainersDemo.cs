...
namespace CosmosDb.DotNetSdk.Demos
{
	...
	public static class ContainersDemo
	{
		public async static Task Run()
		{
			...
			await ViewContainers();// display all containers inside database
		...
		private static async Task ViewContainers()
		{
			...
			var database = Shared.Client.GetDatabase("mydb");//or adventure-works database
		...
		private async static Task ViewContainer(ContainerProperties containerProperties)
		{
			...
			var throughput = await container.ReadThroughputAsync();//to know the provisioned throughput on a container
		...
		private async static Task CreateContainer(
			string containerId,
			int throughput = 400,
			string partitionKey = "/partitionKey")//defaults
		{
			Console.WriteLine();
